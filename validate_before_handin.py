...
#####      (THAT IS, ALL YOUR PROGRAMS, YOUR TOURS AND YOUR PROFORMA - EVERYTHING YOU ARE TO HAND IN)
...
def collect_data_from_program_file(program_file, list_of_programs):
    ...
    if not program_file in list_of_programs:
        flag.append((batch, 1))
...
def collect_data_from_tour_file(tour_file, list_of_tours):
    ...
    flag = [(batch, 0)]
    tour_file_data = ["", "", "", "", 0, 0, []]
    if not tour_file in list_of_tours:
        flag.append((batch, 1))
        tour_file_data[0] = "N"
...
def build_student_feedback_file(user_name, student_record, programs_to_submit, programs_prefix, tour_files_to_submit,
    ...
    f = open(user_name + "_validation_feedback.txt", "w")
    f.write("Feedback for user-ID " + user_name + "\n\n")
    f.write("Throughout this validation, as many error messages as possible are returned in order to help you understand what might have gone wrong.\n")
    f.write("However, note that sometimes a single error might lead to early termination of the validation process so that fixing this error results\n")
    f.write("in more, as yet unseen errors. So, the list of error messages might not be exhaustive but its the best I can do.\n")
    f.write("\n")
    f.write("The first part of this feedback is validation of the program files that you submitted, in terms of the key information they need to contain.\n")
    f.write("At this stage, the programs are not executed; that will follow later. Please note that for any program cluster, if any of\n")
    f.write("the following are true then the program cluster is INVALID and neither it nor the tours it produces will secure any marks:\n")
    ...
    f.write("Also, if you have two valid program clusters but they have the same algorithm code then both clusters are invalid.\n")
    f.write("(Of course, invalidity can be avoided if you check your programs using this program prior to submission!)\n")
    f.write("\n")
    ...
    f.write("\n")
    f.write("The next part of this feedback is validation of the tour files that you submitted. Each has been validated and the outcome is described below.\n")
    f.write("If any tour has an associated error message then it is deemed to be invalid and it will secure no marks. Remember: as stated above, any tours\n")
    f.write("arising from an invalid program cluster will not secure any marks (irrespective of whether they are actually legal tours or not). The tour\n")
    f.write("lengths of successfully validated tours given below go to form your tour-quality mark.\n")
    f.write("\n")
    ...
    for i in range(0,num_batches):
        ...
        for item in tour_files_to_submit[i]:
            f.write("#####      - " + item + ": ")
            if len(student_record[item][1]) == 1:
                f.write("a good tour of length " + str(student_record[item][0][5]) + " ("
...
for item in list_of_contents:
    if item[len(item) - 3:len(item)] == ".py":
        list_of_programs.append(item)
list_of_tours = []
for item in list_of_contents:
    if item[len(item) - 4:len(item)] == ".txt":
        list_of_tours.append(item)
list_of_additionals = []
for item in list_of_contents:
    if item[len(item) - 3:len(item)] != ".py" and item[len(item) - 4:len(item)] != ".txt":
        list_of_additionals.append(item)
...
for item in additional_items_to_submit:
    if item in list_of_additionals:
